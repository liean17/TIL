...
흔히 많은 스터디들에서 자기가 한 주에 공부한 내용을 설명하는 식으로 발표를 많이 한다고 들어서 '아 역시 남한테 설명하는 것 만큼 좋은 공부가 또 없지'라는 생각으로 순서를 기다리고 있었는데,  
...
물론 여기서 이미 아시는 사이트가 많을거에요. 왜냐하면 저도 공부를 하는 입장이기 때문입니다  
...
3. 개발지식
4. 알고리즘
5. 취업 - 취업인데 진짜 별건 아니구요 취업을 잘한 분들의 자료를 볼겁니다.
    - 기술면접  
    - 이력서
    - 블로그
...
## 1. 자바/스프링  
이미 배웠고, 배우고있죠? 짧게 넘어가겠습니다.

### 자바  
- 자바의 정석 : 남궁성  
너무 뻔하죠? 혹시나 모르시는 분이 있을까봐 짧게 말씀드리고 지나가겠습니다.  
앞으로 많은 자바/스프링 개발자 분들의 자료를 말씀드릴건데 거의 대부분 이 책으로 공부를 하셨더라구요  
인프런 스프링강의를 지배하고 계신 김영한님은 배민에서 자바스터디를 만들었는데 교재로 이 책을 골랐다고해요  
책 제목처럼 자바에 대한 정말 많은 내용이 담겨있어서 오히려 처음보는 사람한테는 부담스럽다는 후기도 많아요  
하지만 이제 저희는 어느정도 배웠기에 스프링 공부를 하다 자바 지식이 부족하다고 느끼신다면 책을 사시면 좋고  
특히 강의가 무료인데 책없어도 문제없이 들을 수 있어서 책 구매전에 보시는 것도 추천드립니다.  
또 저자의 네이버카페에 연습문제가 무료로 올라와있기때문에 자바를 익히기 위한 문제가 필요하다고 하시면 추천드립니다.
...
### 스프링
- 인프런 김영한  
이것도 짧게 넘어갈게요, 스프링 대한민국 1타 강사로 알고있습니다  
많은 개발자의 스승이시죠, 19년도 이후로 스프링관련해서 블로그 정리되어있는 글을 보면 대부분 영하님 강의를 정리한게 많습니다.  
초보 개발자 뿐만아니라 현업에서 일하고있는 분들도 많이 들으시는것 같더라구요  
...
개발자 커뮤니티 같은곳에서 보면 토이프로젝트가 굉장히 창의적이고 대단한걸 해야한다는 생각을 하시는 분을 꽤 봤는데  
...
겪은 시행착오에 대해서 설명할 수 있는게, 더 많은 어필이 된다고 하니까  
토이프로젝트로 뭘 만들어볼지 감도 안온다고 하시는 분들이 계시다면
아래 소개해드린 컨텐츠들을 보고 간단한 게시판부터 구현해보시면 좋을 것 같아요

- 스프링 부트와 AWS로 배포하기  
배민 리드개발자셨고 지금은 인프런 CTO이신 동욱님의 책이입니다.  
이미 보고계신분도 여기 많지만 빠르면 며칠만에 한 프로젝트를 경험할 수 있는 책이에요  
그래서 이론만 배울게아니라 뭐든 하나 만들어보고싶다하면 정말 추천드리는 책입니다  
근데 책을 보면서 느낀게 완전히 개발 초보를 위한 책은 아니라서  
아예 개발 경험이 없으면 이게 왜 이렇지? 여기 설명은 왜 안하지? 라는 생각이 들 순 있을것 같아요  
근데 또 모르지만 웹어플리케이션 개발 경험을 한다는 자체가 앞으로 공부의 자산이 되기도 하고  
모르는 부분을 찾아보는 공부가 효율이 더 좋기도 해서
단기간에 웹개발경험을 하고싶다하시면 추천드립니다  

- 호돌맨의 요절복통개발쇼  
어떻게보면 위 책의 강의버전이라고 할 수 있는데  
강사님말로는 이게 강의가 아니래요  
그냥 진짜 웹어플리케이션이 어떤 과정으로 개발되는건지 직접 지켜보고
내가 공부해야하는게 뭔지 깨닫기 위한 강의라고 하는데  
아 개발쇼라고 한것도 스스로 강의라고 생각하지 않아서 그렇게 지었데요  
왜이리 무책임해 라는 생각이 들기도하는데  
...
토이프로젝트 관련 글을 추가하면 좋을듯?


---
## 3. 개발지식  
개발지식이라고 적었는데 대부분 컴퓨터공학 지식 입니다  
저는 비전공자라서 컴퓨터 전반에 대한 이해가 부족하다보니 정보를 많이 찾았구요  
전공하신분들에게는 어떨지 모르겠는데  
...
다음 조언  
인프런 멘토링서비스  
사실 받을까말까 고민하다 코드스테이츠들어오면서 안한건데  
